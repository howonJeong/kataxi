...
const ADJECTIVES = [
    ...
    "서글서글한","시니컬한","무덤덤한","고참급","실세인","운좋은","꼬인","풀린","피곤한","졸린",
    "배고픈","출출한","가식없는","털털한","호탕한","화끈한","뒤끝없는","야무진","어리바리한","멍때리는",
    "눈치빠른","영악한","발칙한","대담한","배짱좋은","헐렁한","느긋한","여유로운","무심한","까칠한",
    "꼬질꼬질한","뽀송한","각잡은","광낸","칼같은","가벼운","묵직한","듬직한","우직한","엉뚱한",
    "골때리는","비범한","평범한","화려한","반짝이는","깔끔한","반듯한","삐딱한","날카로운","둥글둥글한",
    "깐깐한","넉살좋은","짠한","치밀한","단단한","독한","순박한"
];
const ANIMALS = [
    "이등병","일병","상병","병장","하사","중사","상사","소위","중위","대위",
    "행보관","중대장","대대장","당직사관","당직병","취사병","의무병","통신병","운전병","행정병",
    "작전병","피엑스병","말년병장","관심병사","깍두기","일수","실세","에이스","고문관"
];
...
(async () => {
    ...
    const add  = async (col, def) => {
        if (!cols.includes(col)) await db.exec(`ALTER TABLE users ADD COLUMN ${col} ${def}`);
    };
    await add('email',        'TEXT');
    await add('kakaoId',      'TEXT');
    await add('authProvider', "TEXT DEFAULT 'local'");
    await add('name',         'TEXT');
...
async function broadcastRooms() {
    try {
        ...
        const limit = new Date(kst.getTime() - 3 * 3_600_000);
        ...
        const rooms = await db.all(`
            SELECT r.*,
                ...
                (SELECT GROUP_CONCAT(p.userId)
                    FROM participants p WHERE p.roomId = r.id) AS participantList,
                (SELECT GROUP_CONCAT(COALESCE(u.name,'?'))
                    FROM participants p
                    LEFT JOIN users u ON u.userId = p.userId
                    WHERE p.roomId = r.id) AS participantNames
            FROM rooms r
        ...
        console.error('[broadcastRooms]', e);
    }
}

// ── 세션 유틸 ──
const SESSION_TTL = 30 * 24 * 3_600_000; // 30일
...
app.get('/api/check-id', async (req, res) => {
    ...
    res.json(exists
        ? { available: false, reason: '이미 사용 중인 아이디입니다.' }
        : { available: true });
});
...
app.post('/api/kakao-register', async (req, res) => {
    ...
    // ✅ FIX: kakao-register는 토큰을 소비하지 않음 (verify에서 세션 발급)
    const row = await db.get(
    ...
    try {
        ...
        // 토큰은 소비하지 않음 — 이어지는 /api/kakao-verify 에서 소비
        res.json({ ok: true });
    } catch (e) {
        console.error(e);
...
app.get('/auth/kakao/callback', async (req, res) => {
    ...
    try {
        ...
        if (!tokenData.access_token) throw new Error('No access token received from Kakao');
        ...
        if (!user) {
            ...
            if (!user) {
                ...
                console.log('[KAKAO CREATE USER]', { uid, kakaoEmail, kakaoName, kakaoPhone, kakaoBirth });
                await db.run(
                    `INSERT INTO users
                        (userId, email, kakaoId, authProvider, name, phone, birthdate, isOnboarded)
                     VALUES (?, ?, ?, 'kakao', ?, ?, ?, 0)`,
                    [uid, kakaoEmail, kakaoId, kakaoName, kakaoPhone, kakaoBirth]
                );
            ...
            if (kakaoName  && !user.name)      { updates.push('name=?');      params.push(kakaoName); }
            if (kakaoPhone && !user.phone)      { updates.push('phone=?');     params.push(kakaoPhone); }
            if (kakaoBirth && !user.birthdate)  { updates.push('birthdate=?'); params.push(kakaoBirth); }
            if (updates.length) {
                params.push(user.userId);
                await db.run(`UPDATE users SET ${updates.join(', ')} WHERE userId=?`, params);
                user = await db.get(`SELECT * FROM users WHERE userId=?`, [user.userId]);
            }
        }

        // temp token 생성 (10분 유효)
        const tempToken = crypto.randomBytes(32).toString('hex');
        await db.run(
            `INSERT OR REPLACE INTO password_resets (token, userId, expiresAt, used)
             VALUES (?, ?, ?, 0)`,
            [tempToken, user.userId, Date.now() + 10 * 60 * 1000]
        );

        console.log('[KAKAO TEMP TOKEN CREATED]', { userId: user.userId, isOnboarded: user.isOnboarded });
...
app.post('/api/request-reset', async (req, res) => {
    ...
    if (!user || user.authProvider === 'kakao') return res.json({ ok: true }); // 존재 여부 노출 방지
    ...
    try {
        await mailer.sendMail({
            from:    `"KATCHI-TAPSIDA" <${process.env.MAIL_USER}>`,
            to:      email,
            subject: '[KATCHI-TAPSIDA] 비밀번호 재설정 안내',
            html:    `<h2>비밀번호 재설정</h2>
                      <p>아래 링크를 클릭해 30분 이내에 비밀번호를 변경하세요.</p>
                      <a href="${url}" style="background:#4a7fcb;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block;">
                        비밀번호 재설정
                      </a>`,
        });
...
app.post('/api/reset-password', async (req, res) => {
    ...
    await db.run(
        `UPDATE users SET password=?,
            authProvider = CASE WHEN authProvider='kakao' THEN 'both' ELSE authProvider END
         WHERE userId=?`,
...
io.on('connection', (socket) => {
    ...
    socket.on('session_login', async ({ sessionId }) => {
        try {
            ...
            console.error(e);
            socket.emit('login_fail', { reason: 'error' });
        }
    });
    ...
    // 카카오 소켓 인증 (폴백용 — 주 흐름은 REST /api/kakao-verify)
    socket.on('kakao_verify', async ({ token }) => {
        ...
        try {
            ...
            console.error(e);
            socket.emit('error_msg', '카카오 로그인 처리 중 오류가 발생했습니다.');
        }
    });
        ...
        try {
            ...
            if (user.authProvider !== 'kakao') {
                ...
                if (!await bcrypt.compare(currentPw, user.password))
                    return socket.emit('error_msg', '현재 비밀번호가 틀렸습니다.');
            }
            await db.run(
                `UPDATE users SET password=?,
                    authProvider = CASE WHEN authProvider='kakao' THEN 'both' ELSE authProvider END
            ...
            console.error(e);
            socket.emit('error_msg', '비밀번호 변경 중 오류가 발생했습니다.');
        }
    });
    ...
    socket.on('update_email', async ({ email }) => {
        ...
        try {
            ...
            console.error(e);
            socket.emit('error_msg', '이메일 업데이트 중 오류가 발생했습니다.');
        }
    });
    ...
    socket.on('create_room', async ({ date, origin, dest, time, specificLocation, maxPax }) => {
        ...
        if (!origin || !dest || !date || !time)
            return socket.emit('error_msg', '모든 항목을 입력해주세요.');
        if (origin === dest)
            return socket.emit('error_msg', '출발지와 목적지가 같습니다.');

        const today = new Date(Date.now() + 9 * 3_600_000).toISOString().split('T')[0];
        if (date < today)
            return socket.emit('error_msg', '오늘 이후 날짜만 선택할 수 있습니다.');
        ...
        try {
            ...
            const r   = await db.run(
                `INSERT INTO rooms (roomName, creatorId, date, origin, dest, specificLocation, time, maxPax)
                 VALUES (?,?,?,?,?,?,?,?)`,
            ...
            console.error(e);
            socket.emit('error_msg', '방 생성 중 오류가 발생했습니다.');
        }
    });
    ...
    socket.on('join_room', async (roomId) => {
        ...
        try {
            ...
            if (sameDay) return socket.emit('error_msg',
    ...
    socket.on('cancel_join', async ({ roomId }) => {
        ...
        try {
            ...
            if (r.changes > 0) {
                ...
                // 참여자가 0명이면 방 삭제 (CASCADE로 participants도 삭제됨)
    ...
    socket.on('submit_payment', async ({ roomId, amount, bank, account }) => {
        ...
        try {
            ...
            if (isNaN(safe) || safe <= 0 || safe > 10_000_000)
                return socket.emit('error_msg', '올바른 금액을 입력해주세요.');
            if (!account || !bank)
                return socket.emit('error_msg', '계좌 정보를 입력해주세요.');

            const r = await db.run(
                `UPDATE rooms SET payAmount=?, payBank=?, payAccount=?, payerId=? WHERE id=?`,
                [safe, bank, account, socket.userId, roomId]
            );
            if (r.changes > 0) {
                // ✅ FIX: 정산 등록 성공 emit 추가 (HTML에서 payment_registered 수신)
                socket.emit('payment_registered');
                broadcastRooms();
            } else {
                socket.emit('error_msg', '방을 찾을 수 없습니다.');
            }
        } catch (e) {
            console.error(e);
            socket.emit('error_msg', '서버 오류가 발생했습니다.');
    ...
    socket.on('complete_payment', async ({ roomId }) => {
        ...
        try {
            ...
            if (!room.payAmount)
                return socket.emit('error_msg', '정산 금액이 등록되지 않았습니다.');
            ...
            for (const uid of ps) {
                await db.run(
                    ...
                    [uid, room.origin, room.dest, room.date, room.time,
                     room.payAmount, split, ps.length, uid === room.payerId ? 1 : 0]
                );
            }

            // ✅ FIX: rooms 삭제 → CASCADE로 participants 자동 삭제 (중복 DELETE 제거)
            ...
            console.error(e);
            socket.emit('error_msg', '정산 완료 처리 중 오류가 발생했습니다.');
        }
    });
